...
def create_scaler(trial: optuna.Trial) -> tuple[str, BaseEstimator]:
...
def create_power_transformer(trial: optuna.Trial) -> tuple[str, BaseEstimator]:
    """Creates a PowerTransformer step with fixed yeo-johnson method."""
    # No longer tunable to speed up trials
...
@register_step("pca")
def create_pca(trial: optuna.Trial, X) -> tuple[str, BaseEstimator]:
    """Creates a PCA step with direct n_components selection."""
    # Skip variance ratio estimation to speed up trials
    # Use a fixed number of components based on dataset size
...
def create_lasso_selection(trial: optuna.Trial) -> tuple[str, BaseEstimator]:
    """Creates a SelectFromModel step using Lasso with a fixed alpha."""
    # Use a fixed Lasso with small alpha for feature selection to speed up trials
...
def create_linear_regression(trial: optuna.Trial) -> tuple[str, BaseEstimator]:
    """Creates a LinearRegression model step."""
    return ("model", LinearRegression())


@register_step("ridge")
def create_ridge(trial: optuna.Trial) -> tuple[str, BaseEstimator]:
...
def create_random_forest(trial: optuna.Trial) -> tuple[str, BaseEstimator]:
    """Creates a RandomForestRegressor model step with limited hyperparameters."""
    # Very limited hyperparameter space to speed up trials
...
def create_k_neighbors(trial: optuna.Trial) -> tuple[str, BaseEstimator]:
    ...
    p = trial.suggest_int("knn__p", 1, 5)  # only if minkowski
    if metric == "minkowski":
...
def create_pipeline(trial: optuna.Trial, X) -> Pipeline:
    """
    ...
    steps.append(create_scaler(trial))
